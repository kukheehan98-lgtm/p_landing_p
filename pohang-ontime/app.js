...
const selectionKey=location.hostname==='kukheehan98-lgtm.github.io'?'ontime.likes':'culturepick.candidate.likes';
...
const defaultHeading=$('#programs h2').textContent;
...
/* 기관별 확인일을 한 문장으로 요약합니다. 사용자가 알아야 할 것은 기관별 날짜
   목록이 아니라 「지금 이 정보를 믿어도 되는가」이고, 뒤처진 기관이 있을 때만
   그 이름을 밝히면 충분합니다. 가장 오래된 날짜를 기준일로 삼습니다 — 한 곳이라도
   확인 못 했으면 「오늘」이라고 말하지 않기 위해서입니다. */
function freshnessNote(sources){
 const names={phlib:'포항시립도서관','gbelib-yi':'영일도서관',gsei:'경상북도교육청과학원',phcf:'포항문화재단'};
 const dates=Object.values(sources||{}).filter(Boolean).sort();
 if(!dates.length)return '기관 자료 기준 확인 중';
 const today=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Seoul'}).format(new Date());
 const [,m,d]=dates[0].split('-');
 const label='정보 기준일 '+Number(m)+'월 '+Number(d)+'일';
 const late=Object.entries(sources).filter(([,v])=>v&&v<today).map(([k])=>names[k]||k);
 if(!late.length)return label+' · 오늘 확인';
 if(late.length===1)return label+' · '+late[0]+' 확인 지연';
 return label+' · '+late.length+'개 기관 확인 지연';
}
...
function render(){const labels={all:'전체',kid:'유아',elem:'초등',family:'가족',adult:'어른'};const conditions={weekend:'주말',upcoming:'접수 예정',small:'소수 정원',morning:'오전',outing:'나들이'};$('#programs h2').textContent=(audience==='all'&&filter==='all')?defaultHeading:((audience==='all'?'':labels[audience]+' ')+(conditions[filter]?conditions[filter]+' ':'')+'프로그램');for(const id of selected){if(!programs.find(p=>p.id===id)||!SignupState.eligible(programs.find(p=>p.id===id)))selected.delete(id);}const visible=programs.filter(p=>!['closed','full'].includes(phase(p))&&matches(p));$('#count').textContent=visible.length+'개 프로그램';$('#cards').innerHTML=visible.length?visible.map(cardMarkup).join(''):'<div class="empty">이 조건에 맞는 프로그램이 아직 없어요.<br><small>다른 대상이나 조건을 선택해보세요.</small></div>';renderedSignature=programs.map(p=>phase(p)+SignupState.eligible(p)).join();updateSelection();}
...
$('#cards').addEventListener('click',e=>{const b=e.target.closest('[data-pick]');if(!b)return;const id=b.dataset.pick;if(!SignupState.eligible(programs.find(p=>p.id===id))){render();return;}selected.has(id)?selected.delete(id):selected.add(id);CulturePickRelease.track(selected.has(id)?'like':'unlike',{program_id:id});b.setAttribute('aria-pressed',String(selected.has(id)));b.textContent=selected.has(id)?'✓ 담김':'알림 담기';updateSelection();});
...
try{const results=await SignupAdapter.submit(entry,location.href,{state:submissionState,endpoint:integrationConfig.mode==='production'?integrationConfig.endpoint:'/api/test-signup',production:integrationConfig.mode==='production'});if(results.some(r=>r.uncertain))throw Error('저장 여부 확인이 필요한 강좌가 있습니다. 중복 방지를 위해 재전송하지 않았습니다.');$('#integration-result').textContent=results.length+'개 강좌 '+(integrationConfig.mode==='production'?'신청 전송':'모의 저장 확인')+' · '+results.filter(r=>r.duplicate).length+'건 중복 방지';if(integrationConfig.mode==='production'){const ids=JSON.parse(localStorage.getItem('ontime.submitted')||'[]');localStorage.setItem('ontime.submitted',JSON.stringify([...new Set([...(Array.isArray(ids)?ids:[]),...entry.programs.map(p=>p.id)])]));CulturePickRelease.track('signup',{liked_count:entry.programs.length});}$('#preview-form').reset();selected.clear();$('#form-content').hidden=true;$('#success').hidden=false;render();$('#back-to-list').focus();}catch(error){const unknown=[...selected].some(id=>['uncertain','sending'].includes(submissionState.status(programs.find(p=>p.id===id))));$('#form-error').textContent=unknown?'저장 결과를 확인하지 못했습니다. 중복 신청을 막기 위해 재전송을 보류했습니다. 운영 시트 확인이 필요합니다.':error.message||'저장 실패. 다시 시도해주세요.';}finally{submitting=false;btn.disabled=false;$('#close-form').disabled=false;document.querySelectorAll('[data-remove]').forEach(b=>b.disabled=false);}
};
...
Promise.resolve(window.CulturePickRelease.config).then(c=>{integrationConfig=c;submissionState=SignupState.create(localStorage,c.mode==='production'?'ontime.v2.':'culturepick.candidate.');if(c.mode==='production')CulturePickRelease.prepareUI();if(c.mode==='live-test'){$('.preview-note').textContent='실제 연결 시험 · 지정 번호만 신청 가능';$('#phone-help').textContent='지정하신 본인 번호로만 실제 시트 저장을 시험합니다.';$('#test-failure').hidden=true;$('#basket-dialog .form-foot').textContent='실제 연결 시험 · 지정 번호 한 건';$('#preview-form button[type=submit]').textContent='실제 신청 연결 시험';document.querySelectorAll('.privacy-preview').forEach(el=>el.hidden=true);$('.consent small').textContent='기존 운영 Google 시트에 시험 신청이 저장됩니다. 문자 시험은 지정 번호로만 진행합니다.';$('#success h2').textContent='신청 응답을 확인했어요';$('#success p').innerHTML='<span id="integration-result"></span><br>실제 시트 기록과 문자 예약은 별도로 확인합니다.';}return Promise.resolve({ok:true,json:()=>Promise.resolve(JSON.parse(document.getElementById('programsData').textContent))});}).then(r=>{if(!r.ok)throw Error('data');return r.json();}).then(data=>{programs=data;if(integrationConfig.mode==='production'){const legacy=SignupState.migrateLegacy(localStorage,programs,submissionState);legacy.selected.forEach(id=>selected.add(id));if(legacy.pendingCount)$('.application-tip').textContent='이전에 전송이 끝나지 않은 신청 기록이 있습니다. 중복 신청 전 운영자에게 확인해주세요.';}restoreLinks();render();renderSources();Promise.resolve({sources:JSON.parse(document.getElementById('sourceUpdatedAt').textContent)}).then(m=>{$('.snapshot').textContent=freshnessNote(m.sources);});}).catch(()=>{$('#cards').innerHTML='<p class="empty">자료를 불러오지 못했어요. 미리보기 주소로 다시 열어주세요.</p>';});
async function refreshData(){if(!programs.length||submitting||$('#signup-dialog').open||$('#basket-dialog').open)return;let changed=false;try{const r=await fetch('data/programs.json',{cache:'no-store'});if(r.ok){const next=await r.json();if(Array.isArray(next)&&next.length){changed=JSON.stringify(next)!==JSON.stringify(programs);programs=next;}}}catch{}const current=programs.map(p=>phase(p)+SignupState.eligible(p)).join();if(changed||renderedSignature!==current||[...selected].some(id=>!SignupState.eligible(programs.find(p=>p.id===id))))render();}
