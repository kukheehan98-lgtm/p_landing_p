...
async function submit(entry,ref,options={}){const results=[];for(const fields of buildPayloads(entry,ref)){const p=JSON.parse(fields.programs)[0],prior=options.state?.status(p);if(['confirmed','legacy','sent','uncertain','sending'].includes(prior)){results.push({ok:['confirmed','legacy','sent'].includes(prior),duplicate:true,uncertain:!['confirmed','legacy','sent'].includes(prior)});continue;}
 options.state?.mark(p,'sending');
 try{const res=await fetch(options.endpoint||'/api/test-signup',{method:'POST',...(options.production?{mode:'no-cors'}:{}),body:new URLSearchParams(fields)});if(options.production){options.state?.mark(p,'sent');results.push({ok:true,verified:false});continue;}const data=await res.json();if(!res.ok||!data.ok){options.state?.mark(p,data.uncertain?'uncertain':'failed');const error=Error(data.message||'저장 실패');error.handled=true;throw error;}options.state?.mark(p,'confirmed');results.push(data);}catch(error){if(!error.handled)options.state?.mark(p,'uncertain');error.results=results;throw error;}}
return results;}
